...
tea_val_t tea_interpret_evaluate_binop(
  tea_ctx_t* context, tea_scope_t* scope, const tea_node_t* node)
{
  const tea_val_t lhs_val = tea_eval_expr(context, scope, node->binop.lhs);
  const tea_val_t rhs_val = tea_eval_expr(context, scope, node->binop.rhs);

  return tea_val_binop(lhs_val, rhs_val, node->tok);
}

tea_val_t tea_interpret_evaluate_unary(
  tea_ctx_t* context, tea_scope_t* scope, const tea_node_t* node)
{
  rtl_list_entry_t* entry = rtl_list_first(&node->children);
  const tea_node_t* operand = rtl_list_record(entry, tea_node_t, link);
  tea_val_t operand_val = tea_eval_expr(context, scope, operand);
...
tea_val_t tea_interpret_evaluate_ident(const tea_scope_t* scope, const tea_node_t* node)
{
  const tea_tok_t* token = node->tok;
  if (!token) {
  ...
  const tea_var_t* variable = tea_scope_find(scope, token->buf);
  if (!variable) {
...
tea_val_t tea_eval_expr(tea_ctx_t* context, tea_scope_t* scope, const tea_node_t* node)
{
  if (!node) {
  ...
  switch (node->type) {
    ...
    case TEA_N_FN_CALL:
      return tea_eval_fn_call(context, scope, node);
    case TEA_N_STRUCT_INST:
      return tea_eval_new(context, scope, node);
    case TEA_N_FIELD_ACC:
      return tea_eval_field_acc(context, scope, node);
    case TEA_N_NULL:
      return tea_val_null();
    default: {
      ...
      if (token) {
        rtl_log_err(
          ...
          "%d, column %d",
          tea_node_type_name(node->type), tea_tok_name(token->type), token->size, token->buf,
