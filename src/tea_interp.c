...
void tea_interp_cleanup(const tea_ctx_t* ctx)
{
  ...
  {
    tea_struct_decl_t* struct_declaration = rtl_list_record(entry, tea_struct_decl_t, link);
    rtl_list_remove(entry);

    rtl_list_entry_t* function_entry;
    rtl_list_entry_t* function_entry_safe;
    rtl_list_for_each_safe(function_entry, function_entry_safe, &struct_declaration->fns)
    {
