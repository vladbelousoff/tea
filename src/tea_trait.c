...
bool tea_interp_trait_impl(tea_ctx_t* ctx, const tea_node_t* node)
{
  ...
  tea_trait_decl_t* trait_declaration = tea_find_trait_decl(ctx, trait_name_token->buf);
  if (!trait_declaration) {
  ...
  tea_struct_decl_t* struct_declaration = tea_find_struct_decl(ctx, struct_name_token->buf);
...
tea_trait_impl_t* tea_find_trait_impl(
  const tea_ctx_t* ctx, const char* trait_name, const char* struct_name)
{
  rtl_list_entry_t* entry;
...
const tea_fn_t* tea_resolve_trait_method(
  const tea_ctx_t* ctx, const char* struct_name, const char* method_name)
{
