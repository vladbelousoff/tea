...
bool tea_interp_trait_decl(tea_ctx_t *ctx, const tea_node_t *node)
{
  ...
  rtl_list_add_tail(&ctx->traits, &trait_declaration->link);
    ...
    if (child->type == TEA_N_TRAIT_METHOD) {
      ...
      if (function_entry) {
        ...
        if (function_node && function_node->type == TEA_N_FN) {
          const bool result =
            tea_decl_fn(function_node, &trait_declaration->methods);
          if (!result) {
            return false;
          }
        }
...
{
  rtl_list_entry_t *entry;
  rtl_list_for_each(entry, &ctx->traits)
  {
    tea_trait_decl_t *declaration =
      rtl_list_record(entry, tea_trait_decl_t, link);
    if (!declaration) {
...
bool tea_interp_trait_impl(tea_ctx_t *ctx, const tea_node_t *node)
{
  ...
  if (!first_child_entry) {
    rtl_log_err(
      "Runtime error: Trait implementation must specify a struct name");
    return false;
  }

  const tea_node_t *struct_name_node =
    rtl_list_record(first_child_entry, tea_node_t, link);
  ...
  tea_trait_decl_t *trait_declaration =
    tea_find_trait_decl(ctx, trait_name_token->buf);
  if (!trait_declaration) {
    rtl_log_err("Runtime error: Cannot implement undefined trait '%s'",
                trait_name_token->buf);
    return false;
  }

  // Verify the struct exists
  tea_struct_decl_t *struct_declaration =
    tea_find_struct_decl(ctx, struct_name_token->buf);
  if (!struct_declaration) {
    rtl_log_err(
      "Runtime error: Cannot implement trait '%s' for undefined struct '%s'",
  ...
  rtl_list_add_tail(&ctx->impls, &trait_impl->link);

  rtl_log_dbg("Implement trait '%s' for struct '%s'", trait_impl->trait_name,
...
{
  rtl_list_entry_t *entry;
  rtl_list_for_each(entry, &ctx->impls)
  {
    tea_trait_impl_t *impl = rtl_list_record(entry, tea_trait_impl_t, link);
    if (!impl) {
      rtl_log_err(
        "Internal error: Trait implementation list contains null entry");
      continue;
    }
    if (impl->trait_name && impl->struct_name &&
        !strcmp(impl->trait_name, trait_name) &&
