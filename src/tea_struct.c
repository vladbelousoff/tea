...
bool tea_interp_struct_decl(tea_ctx_t* context, const tea_node_t* node)
{
  ...
  rtl_list_add_tail(&context->structs, &struct_declaration->link);
...
tea_struct_decl_t* tea_find_struct_decl(const tea_ctx_t* context, const char* name)
{
  rtl_list_entry_t* entry;
  rtl_list_for_each(entry, &context->structs)
  {
    tea_struct_decl_t* declaration = rtl_list_record(entry, tea_struct_decl_t, link);
    if (!declaration) {
    ...
    const tea_tok_t* declaration_token = declaration->node->tok;
    if (declaration_token) {
      if (!strcmp(declaration_token->buf, name)) {
        return declaration;
      }
    }
  }
...
bool tea_interp_impl_blk(const tea_ctx_t* context, const tea_node_t* node)
{
  ...
  tea_struct_decl_t* struct_declaration = tea_find_struct_decl(context, block_name->buf);
  if (!struct_declaration) {
    rtl_log_err(
      "Runtime error: Cannot implement methods for undeclared struct '%s'", block_name->buf);
    return false;
  }
...
tea_val_t tea_eval_new(tea_ctx_t* context, tea_scope_t* scope, const tea_node_t* node)
{
  ...
  const tea_struct_decl_t* struct_declr = tea_find_struct_decl(context, struct_name->buf);

  tea_inst_t* object = rtl_malloc(sizeof(tea_inst_t) + struct_declr->field_cnt * sizeof(tea_val_t));
  if (!object) {
    rtl_log_err(
  ...
  {
    const tea_node_t* declr_node = rtl_list_record(declr_entry, tea_node_t, link);
    if (!declr_node) {
    ...
    const tea_node_t* field_node = rtl_list_record(field_entry, tea_node_t, link);
    ...
    if (declr->size != field->size || strncmp(declr->buf, field->buf, field->size) != 0) {
      rtl_log_err(
    ...
    tea_val_t value_expr = tea_eval_expr(context, scope, value_node);
    if (value_expr.type == TEA_V_UNDEF) {
      rtl_free(object);
      return tea_val_undef();
    }
    memcpy(&object->buf[field_index++ * sizeof(tea_val_t)], &value_expr, sizeof(tea_val_t));
...
tea_val_t* tea_get_field_ptr(
  const tea_ctx_t* context, const tea_scope_t* scope, const tea_node_t* node)
{
  ...
  if (variable->val.type != TEA_V_INST) {
    rtl_log_err(
      ...
      "(line %d, col %d)",
      object_name->buf, tea_val_type_str(variable->val.type), object_name->line, object_name->col);
    return NULL;
  }
  const tea_inst_t* object = variable->val.obj;

  // Find the struct declaration for this object type
  const tea_struct_decl_t* struct_declr = tea_find_struct_decl(context, object->type);
  if (!struct_declr) {
    rtl_log_err(
  ...
  rtl_list_for_each_indexed(field_index, field_entry, &struct_declr->node->children)
  {
    ...
    const tea_node_t* field_decl_node = rtl_list_record(field_entry, tea_node_t, link);
...
tea_val_t tea_eval_field_acc(
  const tea_ctx_t* context, const tea_scope_t* scope, const tea_node_t* node)
{
  const tea_val_t* result = tea_get_field_ptr(context, scope, node);
  if (result) {
    return *result;
  }
