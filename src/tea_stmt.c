...
bool tea_exec_stmt(tea_ctx_t* ctx, tea_scope_t* scp, const tea_node_t* node, tea_ret_ctx_t* ret_ctx,
...
static bool tea_check_field_mutability(const tea_scope_t* scp, const tea_node_t* field_access_node)
{
...
bool tea_interpret_assign(tea_ctx_t* ctx, tea_scope_t* scp, const tea_node_t* node)
{
...
bool tea_exec_if(tea_ctx_t* ctx, tea_scope_t* scp, const tea_node_t* node, tea_ret_ctx_t* ret_ctx,
  tea_loop_ctx_t* loop_ctx)
{
...
bool tea_exec_while(
  tea_ctx_t* ctx, tea_scope_t* scp, const tea_node_t* node, tea_ret_ctx_t* ret_ctx)
{
  ...
  rtl_list_for_each(child_entry, &node->children)
  {
    ...
    switch (child->type) {
      case TEA_N_WHILE_COND:
        cond = rtl_list_record(rtl_list_first(&child->children), tea_node_t, link);
        break;
...
bool tea_exec(tea_ctx_t* ctx, tea_scope_t* scp, const tea_node_t* node, tea_ret_ctx_t* ret_ctx,
  tea_loop_ctx_t* loop_ctx)
{
  if (ret_ctx && ret_ctx->is_set) {
    return true;
    ...
    default: {
      ...
      if (token) {
        rtl_log_err(
          ...
          "(line %d, col %d)",
          tea_node_type_name(node->type), ctx->fname, tea_tok_name(token->type), token->size,
