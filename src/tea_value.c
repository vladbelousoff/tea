...
{
  static tea_type_string_id ids[] = { { "i32", TEA_V_I32 },
                                      { "f32", TEA_V_F32 },
                                      { "string", TEA_V_INST },
                                      { NULL, TEA_V_UNDEF } };
...
tea_val_t tea_val_binop(const tea_val_t lhs_val, const tea_val_t rhs_val,
                        const tea_tok_t *op)
{
  tea_val_t result;
  ...
  rtl_log_err(
    ...
    tea_val_type_str(rhs_val.type), op->line, op->col);
  return tea_val_undef();
}
